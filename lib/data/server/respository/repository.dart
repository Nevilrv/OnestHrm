...
class Repository {
  ...
  static Future<ApiResponse<ResponseCheckOut>> checkOutApi(
    ...
    try {
      EasyLoading.show(status: 'loading...');
      var response = await ApiService.getDio()!
          .patch("/user/attendance/check-out/$checkInID", data: bodyCheckOut);
      EasyLoading.dismiss();
      if (response.statusCode == 200) {
        if (kDebugMode) {
          print(response.data);
  ...
  static Future<ApiResponse<AttendanceDailyReport>> attendanceReportDaily(
    ...
    try {
      EasyLoading.show(status: 'loading...');
      var response = await ApiService.getDio()!
          .post("/report/attendance/particular-date", data: bodyDailyReport);
      EasyLoading.dismiss();
      if (response.statusCode == 200) {
        if (kDebugMode) {
          print(response.data);
  ...
  static Future<ApiResponse<ResponseBaseSetting>> baseSettingApi() async {
    try {
      // EasyLoading.show(status: 'loading...');
      // https://hrm.udaycredit.com/api/V11/app/base-settings
      var response = await ApiService.getDio()!.get("/app/base-settings");
      // EasyLoading.dismiss();
      if (response.statusCode == 200) {
        print('------------------base setting api call');
        if (kDebugMode) {
          print('base setting response===  ${response.data}');
        }
      ...
      if (e.type == DioErrorType.response) {
        ...
        EasyLoading.dismiss();
        if (kDebugMode) {
          print('baseSettingApi error ============${e.message}');
        }
        return ApiResponse(
            httpCode: -1, message: "Connection error ${e.message}");
      }
  ...
  Future<bool> sentStopLocationData(
    ...
    try {
      ...
      EasyLoading.show(status: 'loading...');
      var response = await ApiService.getDio()!
          .post("/user/attendance/live-location-store", data: data);
      EasyLoading.dismiss();
      if (response.statusCode == 200) {
        if (kDebugMode) {
