...
class AppointmentRepository {
  static Future<ApiResponse<ResponseMeetingList>> postAppointmentList(
      data) async {
    try {
      ...
      var response =
          await ApiService.getDio()!.post("/appoinment/get-list", data: data);
      // EasyLoading.dismiss();
      if (response.statusCode == 200) {
        print('--------------- appoinment get list api call');
        if (kDebugMode) {
          print(response.data);
        }
  ...
  ///Appointment Details
  static Future<ApiResponse<AppointmentDetailsModel>> getAppointmentDetails(
      id) async {
    try {
      EasyLoading.show(status: 'loading...');
      if (kDebugMode) {
