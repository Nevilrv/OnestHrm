...
class VisitDetailsProvider extends ChangeNotifier {
  ...
  // location(String? latitudeServer, String? longitudeServer) async {
  //   var lat = double.parse(latitudeServer.toString());
  //   var log = double.parse(longitudeServer.toString());
  //   List pm = await placemarkFromCoordinates(lat, log);
  //   Placemark placeMark = pm[0];
  //   youLocationServer =
  //       "${placeMark.street ?? ""},${placeMark.locality ?? ""} ${placeMark.postalCode ?? ""}";
  ...
  changeStatusVisitApi() async {
    ...
    if (apiResponse.result == true) {
      ...
      notifyListeners();
      // NavUtil.navigateScreen(context, VisitDetails(visitID: visitId,));
    } else {
      Fluttertoast.showToast(
          msg: apiResponse.message ?? "",
          toastLength: Toast.LENGTH_SHORT,
